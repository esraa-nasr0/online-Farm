          ...
          animalBreeding.map((breeding, index) => (
            <div className="mating-record-item" key={breeding._id}>
              <div className="mating-record-info">
                ...
                {breeding.tagId && (
                  <p>
                    <strong>Tag ID:</strong> {breeding.tagId}
                  </p>
                )}
                {breeding.deliveryDate && (
                  <p>
                    <strong>Delivery Date:</strong>{" "}
                    {breeding.deliveryDate.slice(0, 10)}
                  </p>
                )}
                {breeding.deliveryState && (
                  <p>
                    <strong>Delivery State:</strong> {breeding.deliveryState}
                  </p>
                )}
                {breeding.motheringAbility && (
                  <p>
                    <strong>Mothering Ability:</strong>{" "}
                    {breeding.motheringAbility}
                  </p>
                )}
                {breeding.milking && (
                  <p>
                    <strong>Milking:</strong> {breeding.milking}
                  </p>
                )}
                {breeding.numberOfBirths != null && (
                  <p>
                    <strong>Number of Births:</strong>{" "}
                    {breeding.numberOfBirths}
                  </p>
                )}

                {/* عرض كل الـ Birth Entries */}
                {breeding.birthEntries?.length > 0 && (
                  <>
                    <p>
                      <strong>Birth Entries:</strong>
                    </p>
                    <ul className="birth-entries-list pl-4 list-disc">
  {breeding.birthEntries.map((entry) => (
    <React.Fragment key={entry._id}>
      {entry.tagId && <li>Tag: {entry.tagId}</li>}
      {entry.gender && <li>Gender: {entry.gender}</li>}
      {entry.birthweight != null && <li>Weight: {entry.birthweight} kg</li>}
      {entry.expectedWeaningDate && (
        <li>Expected Weaning Date: {entry.expectedWeaningDate.slice(0, 10)}</li>
      )}
      <li>Created: {entry.createdAt?.slice(0, 10)}</li>
    </React.Fragment>
  ))}
</ul>
